...
class RepeatPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.repeat_group_whitelist = config.get("repeat_group_whitelist",[]) # 群白名单导入
        self.repeat_user_whitelist = config.get("repeat_user_whitelist",[]) # 用户白名单导入
        self.repeat_words_blacklist = config.get("repeat_words_blacklist",[]) #屏蔽词导入
    ...
    async def repeat(self, event: AstrMessageEvent):
        ...
        if self.repeat_group_whitelist and group_id not in self.repeat_group_whitelist: #群白名单过滤
            # print(f"群{group_id}不在白名单中")
            return
        if self.repeat_users: #用户白名单过滤
            user_name = next((value for key,value in self.repeat_users if key == sender_id),None)
        ...
        for word in self.repeat_words_blacklist: #屏蔽词过滤
            if word in event.message_str:
                ...
                await event.send(MessageChain([Comp.Plain("触发屏蔽词"),username_chain]))
                return

        print(f"raw_message:{message.raw_message}") #平台下发的原始消息
        # print(f"message:{chain}") #消息链

        chain_type = str(chain[0].type).split(".")[-1] #获取第一段消息类型 用于判断复读逻辑
        if chain_type == "Forward":
            chain = [Comp.Plain("[聊天记录]")]
        elif chain_type == "Image" and message.raw_message['message'][0]['data']['sub_type'] == 0: #不过滤动画表情
            chain = [Comp.Plain("[图片]")]
        ...
        random_time = random.random() * 5000 + random.random() * 2000 + random.random() * 1000 + 1000
